...
// ragardless of update frequency.
//
class Timer
{
public:
	// Create a clock with a perid and the specified clock. If the clock
	...
	// Returns zero if stopped, otherwise returns the time lapsed between the clock's current
	...
	// Return true if our start time is zero.
	bool IsStopped() const;

	// Return true is our elapsed time is greater than our period and we are not stopped.
	...
	// returns false so the caller can process each elapsed preiod.
