...
void AudioSystem::UpdateListener(int listenerIndex, Vec3 const& listenerPosition, Vec3 const& listenerForward, Vec3 const& listenerUp)
{
	...
	FMOD_VECTOR Pos3D, Forward3D, Up3D;
	Pos3D.x = -listenerPosition.y;
	Pos3D.y = listenerPosition.z;
	Pos3D.z = -listenerPosition.x;

	Forward3D.x = -listenerForward.y;
	Forward3D.y = listenerForward.z;
	Forward3D.z = -listenerForward.x;

	Up3D.x = -listenerUp.y;
	Up3D.y = listenerUp.z;
	Up3D.z = -listenerUp.x;

	result = m_fmodSystem->set3DListenerAttributes(listenerIndex, &Pos3D, 0, &Forward3D, &Up3D);
}
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, Vec3 const& soundPosition, bool isLooped /*= false*/, float volume /*= 1.f*/, float balance /*= 0.f*/, float speed /*= 1.f*/, bool isPaused /*= false*/)
{
	...
	FMOD_VECTOR Pos3D;
	Pos3D.x = -soundPosition.y;
	Pos3D.y = soundPosition.z;
	Pos3D.z = -soundPosition.x;
	...
	if (channelAssignedToSound)
	{
		...
		channelAssignedToSound->setPan(balance);
		channelAssignedToSound->setLoopCount(loopCount);
		channelAssignedToSound->set3DAttributes(&Pos3D, 0);
	}
...
{
	FMOD_RESULT result;
	result = m_fmodSystem->set3DNumListeners(numListeners);
	ValidateResult(result);
}
